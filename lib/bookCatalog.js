...
export async function updateBook(bookId, data) {
  ...
  const supplierId = data.supplierId === "" || data.supplierId == null ? null : Number(data.supplierId);
...
// ينشئ كتاب جديد بالكتالوج (باركود + عنوان فقط) — بدون تخزينه على رف
export async function createBook({ barcode, title }) {
  const cleanBarcode = (barcode || "").trim();
  const cleanTitle = (title || "").trim();
  if (!cleanBarcode || !cleanTitle) {
    return { error: { status: 400, body: { error: "الباركود والعنوان كلاهما مطلوب" } } };
  }

  const existing = await prisma.book.findUnique({ where: { barcode: cleanBarcode } });
  if (existing) {
    return { error: { status: 409, body: { error: "هذا الباركود مستخدم لكتاب آخر" } } };
  }

  const book = await prisma.book.create({ data: { barcode: cleanBarcode, title: cleanTitle } });
  return { result: book };
}
