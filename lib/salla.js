...
// بالعميل الصحيح عندنا (الإشعار يحمل merchant، مو معرّفنا الداخلي). غير مؤكد 100% (لا توثيق
// رسمي بمتناول اليد وقت الكتابة) — يفشل بصمت (يرجع null) بدل ما يكسر الربط بالكامل.
export async function fetchMerchantId(accessToken) {
  try {
    ...
    return data?.data?.id || data?.data?.merchant?.id || null;
  } catch {
    return null;
  }
}
