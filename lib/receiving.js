...
// مرتّبة حسب آخر مسحة لكل كتاب (تنازليًا) — عشان يظهر آخر كتاب اتمسح باركوده أول القائمة
export async function getUnlinkedTally() {
  ...
  return tallyRows.map((t) => ({
    ...
    lastScannedAt: t._max.scannedAt,
  }));
}

// راجع قسم 6.2: مسح كل كتاب وصل فعليًا بدون تحديد مورد أو فاتورة مسبقًا
// quantity: مسح دفعي (أكثر من نسخة بمسحة وحدة) — كل نسخة تُسجَّل كسجل ReceivingScan منفصل
// (نفس منطق المسح الفردي المكرَّر تمامًا)، عشان تبقى المطابقة والاعتماد بدون أي تغيير
export async function scanReceiving(barcode, quantity = 1) {
  if (!barcode) return { error: { status: 400, body: { error: "الباركود مطلوب" } } };
  const qty = Math.floor(Number(quantity));
  if (!Number.isFinite(qty) || qty < 1) {
    return { error: { status: 400, body: { error: "الكمية غير صحيحة" } } };
  }
  ...
  await prisma.receivingScan.createMany({ data: Array.from({ length: qty }, () => ({ bookId: book.id })) });

  const tally = await getUnlinkedTally();
  return { result: { book: { title: book.title, barcode: book.barcode }, quantity: qty, tally } };
}
...
// allowShort: تعتمد الفاتورة حتى لو وصلت ناقصة — تعدّل تلقائيًا الكمية المتوقعة لأي بند ناقص
// لتطابق المتوفر فعليًا (بدل ما يضطر الموظف يعدّل كل بند يدويًا)، فيُحاسَب المورد على المستلم فعليًا فقط
export async function approveReceivingInvoice(invoiceId, shelfId, allowShort = false) {
  ...
  if (allowShort) {
    for (const item of invoice.items) {
      ...
      if (available < item.quantityExpected) {
        await prisma.purchaseInvoiceItem.update({ where: { id: item.id }, data: { quantityExpected: available } });
        item.quantityExpected = available;
      }
    }
  }
