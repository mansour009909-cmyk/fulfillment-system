...
// employeeId (اختياري): يُسجَّل "كتاب زائد" كخطأ بسجل أداء الموظف (قسم 2.5) — فقط من الجوال حيث الهوية معروفة
export async function verifyItemScan(orderId, barcode, employeeId) {
  const order = await prisma.order.findUnique({ where: { id: orderId } });
  if (!order) return { error: { status: 404, body: { error: "الطلب غير موجود" } } };
  ...
  if (!item) {
    const detail = `كتاب زائد: "${book.title}" غير موجود ضمن طلب هذا الصندوق أصلًا`;
    if (employeeId) {
      await prisma.employeeErrorLog.create({
        data: { employeeId, type: "EXCESS_ITEM", orderId, bookId: book.id, detail },
      });
    }
    return {
      error: { status: 409, body: { error: detail, excess: true, book: { title: book.title, barcode: book.barcode } } },
    };
  }
  if (item.quantityVerified >= item.quantityRequired) {
    const detail = `كتاب زائد: "${book.title}" — تم مسح الكمية المطلوبة بالكامل مسبقًا`;
    if (employeeId) {
      await prisma.employeeErrorLog.create({
        data: { employeeId, type: "EXCESS_ITEM", orderId, bookId: book.id, detail },
      });
